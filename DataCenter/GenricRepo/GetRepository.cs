...
{
    public class GetRepository<TEntity> : IGetRepository<TEntity> where TEntity : BaseEntity
    {
        ...
        {
            var result = await _context.Set<TEntity>().Where(s=>s.IsDeleted != true && s.Id == id).FirstOrDefaultAsync();
        ...
        {
            var result = await _context.Set<TEntity>().Where(s=>s.IsDeleted != true).ToListAsync();
